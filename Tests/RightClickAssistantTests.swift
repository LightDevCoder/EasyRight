...
final class EasyRightTests: XCTestCase {
    ...
    func testEssentialProfileNeverChangesHighRiskActions() {
        ...
        XCTAssertFalse(states.keys.contains("easyright.action.filemanage.delete"))
        XCTAssertFalse(states.keys.contains("easyright.action.filemanage.moveTo"))
        XCTAssertFalse(states.keys.contains("easyright.action.utility.toggleHiddenFiles"))
    }
    ...
    func testProfessionalProfileEnablesProfessionalButNotAdvancedActions() {
        ...
        XCTAssertEqual(states["easyright.action.newfile.json"], true)
        XCTAssertEqual(states["easyright.action.terminal.vscode"], true)
        XCTAssertNil(states["easyright.action.filemanage.delete"])
    }

    func testActionSearchMatchesTitleAndIdentifier() {
        XCTAssertTrue(ActionSearch.matches(
            title: "新建文本文件",
            actionID: "easyright.action.newfile.txt",
            query: "文本"
        ))
        XCTAssertTrue(ActionSearch.matches(
            title: "新建文本文件",
    ...
    func testFreshSettingsMigrationAppliesEssentialProfile() {
        ...
        XCTAssertTrue(storage.getBool(
            forKey: "enable_action_easyright.action.newfile.txt",
            defaultValue: false
        ))
        XCTAssertFalse(storage.getBool(
            forKey: "enable_action_easyright.action.newfile.json",
            defaultValue: true
        ))
        XCTAssertFalse(storage.getBool(
            forKey: "enable_action_easyright.action.filemanage.delete",
            defaultValue: false
        ))
        XCTAssertTrue(storage.getBool(
    ...
    func testLegacySettingsMigrationPreservesEffectiveStatesAndHighRiskChoice() {
        let actions = DefaultActionRegistry.makeActions()
        XCTAssertTrue(storage.setBool(false, forKey: "enable_action_easyright.action.newfile.txt"))
        XCTAssertTrue(storage.setBool(true, forKey: "enable_action_easyright.action.newfile.json"))
        XCTAssertTrue(storage.setBool(true, forKey: "enable_action_easyright.action.filemanage.delete"))
    ...
    func testApplyingProfileLeavesAdvancedActionChoiceUntouched() {
        let actions = DefaultActionRegistry.makeActions()
        let deleteKey = "enable_action_easyright.action.filemanage.delete"
    ...
    func testSharedActionQueuePreservesMultipleEvents() throws {
        ...
        _ = try storage.enqueueAction(actionId: "easyright.action.newfile.txt", paths: [tempDirectory.path])
        _ = try storage.enqueueAction(actionId: "easyright.action.newfile.md", paths: [tempDirectory.path])
    ...
    func testFinderSyncUsesCacheNoSyncIO() {
        ...
        let actionId = "easyright.action.test.cache.no_sync_io"
    ...
    func testQRCodePasteboardWriterPlacesImage() {
        ...
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("com.easyright.app.test.qr"))
    ...
    func testPluginkitParserDetectsExtensionRegistrationStates() {
        let bundleId = "com.easyright.app.extension"
        ...
        XCTAssertEqual(
            FinderExtensionDiagnostics.registrationState(
                pluginKitOutput: "+    com.easyright.app.extension(1.1.1)",
                commandSucceeded: true,
                bundleIdentifier: bundleId
            ),
            .enabled
    ...
    func testFinderServiceFallbackUsesOneStablePaletteEntry() {
        XCTAssertEqual(FinderServiceCatalog.menuTitle, "EasyRight…")
        XCTAssertEqual(
            FinderServiceCatalog.serviceUserData,
            "com.easyright.service.openActionPalette"
        )
        XCTAssertTrue(FinderServiceCatalog.accepts(userData: FinderServiceCatalog.serviceUserData))
        XCTAssertFalse(FinderServiceCatalog.accepts(userData: "easyright.action.filemanage.cut"))
    }

    func testFinderServiceRequestOnlyAcceptsCurrentlyAuthorizedDirectActions() {
        let actionID = "easyright.action.filemanage.cut"
        ...
        XCTAssertNil(
            FinderServiceCatalog.request(
                userData: "easyright.action.filemanage.delete",
                authorizedDirectActionIDs: [actionID]
            )
        )
    }
    ...
    func testQuickActionsPlaceFavoritesFirstAndFillRecommendationsWithoutDuplicates() {
        let cut = TestMenuAction(
            id: "easyright.action.filemanage.cut",
            title: "剪切",
            category: .fileManage
        )
        let copyPath = TestMenuAction(
            id: "easyright.action.filemanage.copyPath",
            title: "拷贝完整路径",
            category: .fileManage
        )
    ...
    func testQuickServiceManifestAndBundleStoreAreStableAndIdempotent() throws {
        let item = FinderServiceActionItem(
            actionID: "easyright.action.filemanage.cut",
            title: "剪切",
            iconName: "scissors",
            category: .fileManage,
            isFavorite: true,
        ...
        let recommendation = FinderServiceActionItem(
            actionID: "easyright.action.filemanage.copyName",
            title: "拷贝文件名",
        ...
        XCTAssertEqual(
            menuItems,
            ["EasyRight · 1 ★ 剪切", "EasyRight · 2 拷贝文件名"]
        )
    ...
    func testFinderServiceItemSearchMatchesChineseTitleCategoryAndIdentifier() {
        let items = [
            FinderServiceActionItem(
                actionID: "easyright.action.utility.calculateSHA256",
