...
grep -q 'version "0.1.1"' "$CASK_FILE" || fail "Cask 必须使用明确版本"
grep -q 'sha256 "1076120a750a9e98cbf6b998c911cc3a591ef9e8e52fb64da807b06fc3ac19ca"' "$CASK_FILE" || fail "Cask 必须校验稳定制品"
...
grep -q 'Library/Services/EasyRightQuickActions.service' "$CASK_FILE" || fail "Cask 卸载时必须清理动态服务"
...
grep -Fq 'rm -rf "$(brew --caskroom)/easyright"' "$CASK_FILE" || fail "Cask 必须提供 v1.2.0 升级故障恢复命令"
...
grep -Fq 'brew audit --cask --strict --online "$TAP_NAME/easyright"' "$VALIDATOR_FILE" || fail "Cask 验证器必须使用 Homebrew 6 支持的完整限定名称执行 audit"
