...
[ "$(grep -c 'Contents/Resources/EasyRightQuickService' "$WORKFLOW_FILE")" -ge 3 ] || fail "开发构建、发布签名和发布架构必须分别验证快捷服务 helper"
grep -q '"ReleaseWorkflowStructureTests.sh"' Package.swift || fail "SwiftPM 必须排除发布结构测试脚本"
grep -q 'EasyRight-v\${VERSION}-macOS-Universal.dmg' "$WORKFLOW_FILE" || fail "缺少不可变版本 DMG"
grep -q 'EasyRight-Latest.dmg' "$WORKFLOW_FILE" || fail "缺少 Latest DMG 便利资产"
