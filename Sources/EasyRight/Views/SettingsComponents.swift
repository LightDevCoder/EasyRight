...
private let rightClickFinderExtensionBundleIdentifier = "com.easyright.app.extension"
...
func showFinderExtensionRegistrationOutcome(_ outcome: FinderExtensionRegistrationOutcome) {
    if outcome.isSuccess {
        ...
        SharedHUDManager.show(
            title: "注册失败",
            content: outcome.errorDescription ?? "请打开扩展设置手动启用EasyRight 访达扩展",
            isSuccess: false
        )
    }
}
    ...
    private func relaunchAppAndRestartFinder() {
        ...
        PermissionRefreshCoordinator.performReload(
            ...
            guard outcome.isSuccess else {
                SharedHUDManager.show(
                    title: "重新打开失败",
                    content: outcome.relaunchResult?.errorDescription ?? "请手动重新打开EasyRight",
                    isSuccess: false
                )
                return
            }
