...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    private let pendingActionDispatchQueue = DispatchQueue(
        label: "com.easyright.app.pending-dispatch",
        qos: .userInitiated
    )
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: Notification.Name("com.easyright.app.triggerActionSignal"),
            object: nil,
        ...
        // 4. 创建 SwiftUI Studio 主设置视图并托管在 NSWindow 中
        ...
        window = NSWindow(
            contentRect: NSRect(
                x: 0,
                y: 0,
                width: DesignTokens.Layout.windowIdealWidth,
                height: DesignTokens.Layout.windowIdealHeight
            ),
        ...
        print("[App] EasyRight宿主程序启动并初始化完成 (双保险中介链路就绪)")
    ...
    private func runLaunchSelfTest() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 30.0) {
            ...
            print("[App] [SelfTest] 2. 正在发送跨进程空信号 com.easyright.app.triggerActionSignal...")
    ...
    private func handlePermissionRefreshLaunchIfNeeded() {
        ...
        SharedHUDManager.show(
            title: "正在刷新 Finder",
            content: "已重新打开EasyRight，正在让 Finder 按新权限加载右键菜单",
            isSuccess: true
        )
    ...
    @objc func showSettingsWindow() {
        // 转换激活策略为 .regular 模式（显示 Dock 图标并置顶）
    ...
    // MARK: - NSWindowDelegate (常驻后台静默运行生命周期拦截与激活策略切换)
    func windowShouldClose(_ sender: NSWindow) -> Bool {
        // 1. 物理隐藏偏好设置窗口，避免被彻底销毁
        window.orderOut(nil)
        
        // 2. 转换激活策略回 .accessory 模式，移除 Dock 图标常驻后台
        NSApp.setActivationPolicy(.accessory)
    ...
    func windowDidMiniaturize(_ notification: Notification) {
        NSApp.setActivationPolicy(.accessory)
        SharedStorageManager.shared.writeLog("[App] 偏好设置窗口已最小化，宿主程序自动降级为 .accessory 模式")
    }

    func windowDidDeminiaturize(_ notification: Notification) {
        NSApp.setActivationPolicy(.regular)
        NSApp.activate(ignoringOtherApps: true)
        SharedStorageManager.shared.writeLog("[App] 偏好设置窗口从最小化恢复，宿主程序提升为 .regular 模式")
    }
