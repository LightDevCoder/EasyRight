...
public final class ActionDispatcher: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.easyright.app.dispatcher", qos: .userInitiated)
    ...
    public func unregister(actionId: String) {
        queue.sync {
            _ = registeredActions.removeValue(forKey: actionId)
        }
    }

    /// 清空并重新注册指定的一组动作
    public func reset(with actions: [MenuAction]) {
        queue.sync {
            registeredActions.removeAll(keepingCapacity: true)
