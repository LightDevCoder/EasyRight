...
public final class UtilityAction: MenuAction, Sendable {
    ...
    private static let hashRunner = BackgroundActionRunner(
        ...
        ioQueueLabel: "com.easyright.app.utility-hash-io"
    )
    private static let imageConversionRunner = BackgroundActionRunner(
        actionLabel: "utility.image-conversion",
        ioQueueLabel: "com.easyright.app.utility-image-conversion-io"
    )
    ...
    public init(type: UtilityType, imageConverter: any ImageConverterProtocol = DefaultImageConverter()) {
        ...
        self.actionId = "easyright.action.utility.\(type.rawValue)"
...
public final class DefaultImageConverter: ImageConverterProtocol {
    ...
    public func convert(url: URL, toFormat format: String) -> Result<URL, Error> {
        ...
        guard normalizedFormat == "png" || normalizedFormat == "jpeg" || normalizedFormat == "jpg" else {
            return .failure(NSError(domain: "com.easyright.ImageConverter", code: 400, userInfo: [NSLocalizedDescriptionKey: "不支持的目标转换格式：\(format)"]))
        }
        
        guard let source = CGImageSourceCreateWithURL(url as CFURL, nil),
              CGImageSourceGetCount(source) > 0 else {
            return .failure(NSError(domain: "com.easyright.ImageConverter", code: 404, userInfo: [NSLocalizedDescriptionKey: "无法读取或解析输入图片文件"]))
        }
        ...
        guard let destination = CGImageDestinationCreateWithURL(
            ...
            return .failure(NSError(domain: "com.easyright.ImageConverter", code: 500, userInfo: [NSLocalizedDescriptionKey: "无法创建目标图像编码器"]))
        }
        ...
        guard CGImageDestinationFinalize(destination) else {
            ...
            return .failure(NSError(domain: "com.easyright.ImageConverter", code: 500, userInfo: [NSLocalizedDescriptionKey: "生成目标图像文件失败"]))
        }
...
extension UtilityAction {
    ...
    static let toggleHiddenRunner = InteractiveActionRunner(
        ...
        ioQueueLabel: "com.easyright.app.utility-toggle-hidden-io"
    )
    ...
    private static func requireSuccessfulExit(_ process: Process, label: String) throws {
        guard process.terminationStatus == 0 else {
            throw NSError(
                domain: "com.easyright.UtilityAction",
