...
struct ActionVerifier {
    static func main() {
        ...
        let testDirURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("EasyRightVerifier-\(UUID().uuidString)", isDirectory: true)
        ...
        let sharedRootPath = ProcessInfo.processInfo.environment["RIGHTCLICKASSISTANT_SHARED_ROOT"]
            ?? (homeDir as NSString).appendingPathComponent(
                "Library/Containers/com.easyright.app.extension/Data"
            )
        ...
        let requiredProfessionalActions = [
            ...
            "easyright.action.newfile.html",
            "easyright.action.utility.calculateMD5",
            "easyright.action.utility.convertToJPG"
        ]
        ...
        func runTest(name: String, actionId: String, targets: [URL], assertion: () -> Bool) {
            ...
            DistributedNotificationCenter.default().postNotificationName(
                Notification.Name("com.easyright.app.triggerActionSignal"),
                object: nil,
                userInfo: nil,
        ...
        runTest(name: "新建文本文档", actionId: "easyright.action.newfile.txt", targets: [testDirURL]) {
            let fileURL = testDirURL.appendingPathComponent("未命名.txt")
            return FileManager.default.fileExists(atPath: fileURL.path)
        }
        
        runTest(name: "新建 Markdown 目录去重", actionId: "easyright.action.newfile.md", targets: [testDirURL]) {
            // 第一次创建：未命名.md
            ...
            let secondAction: [String: Any] = [
                ...
                "id": UUID().uuidString,
                "createdAt": Date().timeIntervalSince1970,
                "actionId": "easyright.action.newfile.md",
            ...
            DistributedNotificationCenter.default().postNotificationName(
                Notification.Name("com.easyright.app.triggerActionSignal"), object: nil, userInfo: nil, deliverImmediately: true)
            Thread.sleep(forTimeInterval: 2.5)
        ...
        runTest(name: "新建 Word 精简包骨架", actionId: "easyright.action.newfile.docx", targets: [testDirURL]) {
        ...
        runTest(name: "新建 HTML 基础文档", actionId: "easyright.action.newfile.html", targets: [testDirURL]) {
        ...
        runTest(name: "新建 PDF 空白骨架", actionId: "easyright.action.newfile.pdf", targets: [testDirURL]) {
        ...
        runTest(name: "拷贝文件完整物理路径", actionId: "easyright.action.filemanage.copyPath", targets: [copySrcFile]) {
            // 检查 NSPasteboard 里的字符串是否等于该物理路径
            let clipStr = NSPasteboard.general.string(forType: .string) ?? ""
            return clipStr == copySrcFile.path
        }
        
        runTest(name: "拷贝文件名", actionId: "easyright.action.filemanage.copyName", targets: [copySrcFile]) {
            let clipStr = NSPasteboard.general.string(forType: .string) ?? ""
        ...
        runTest(name: "物理计算 MD5 码并写入剪切板", actionId: "easyright.action.utility.calculateMD5", targets: [hashTestFile]) {
            let clipStr = NSPasteboard.general.string(forType: .string) ?? ""
            print("ℹ️ [Verifier] 剪贴板 MD5 结果: \(clipStr)")
            return clipStr == "6f6b4a05c9f02fd24430a09bc98e7759"
        }
        
        runTest(name: "物理计算 SHA256 码并写入剪切板", actionId: "easyright.action.utility.calculateSHA256", targets: [hashTestFile]) {
            let clipStr = NSPasteboard.general.string(forType: .string) ?? ""
        ...
        runTest(name: "图片转换为 JPEG", actionId: "easyright.action.utility.convertToJPG", targets: [pngFile]) {
