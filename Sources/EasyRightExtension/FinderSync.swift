...
class FinderSync: FIFinderSync {
    private static let cutStateChangedNotification = Notification.Name("com.easyright.app.cutStateChanged")
    ...
    private static func getTag(
        ...
        ActionTagMapper.getTag(for: actionId, invocationKind: invocationKind)
    }
    
    private static func getSelection(for tag: Int) -> ActionTagMapper.MenuSelection? {
        ActionTagMapper.getSelection(for: tag)
    }
    ...
    @objc func actionMenuItemSelected(_ sender: NSMenuItem) {
        ...
        guard let selection = ActionTagMapper.getSelection(for: tag) else {
    ...
    override init() {
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: Notification.Name("com.easyright.app.configChanged"),
            object: nil
        )
    ...
    static func ensureHostRunning() {
        let hostBundleID = "com.easyright.app"
    ...
    override func menu(for menuKind: FIMenuKind) -> NSMenu? {
        ...
        let menu = FinderMenuLayoutBuilder.buildNSMenu(
            ...
            itemFactory: { [weak self] action in
                guard let self = self else {
                    return FinderMenuLayoutBuilder.defaultMenuItem(for: action)
                }
                let item = self.makeMenuItem(for: action, invocationKind: invocationKind)
                self.logToSharedContainer("[FinderSync] 成功添加菜单项: [\(action.localizedTitle)]", level: .debug)
                return item
            }
        )
    ...
    private func makeMenuItem(
        ...
        item.tag = Self.getTag(for: action.actionId, invocationKind: invocationKind)
        item.target = self
