...
- **actions**: 新增「自定义外部应用动作 (Custom App Actions)」模块。用户可以在设置中自由添加本地任意已安装软件（如 Keka、Typora、IINA、Sublime Merge 等）至访达右键菜单，实现快捷打开、解压缩或文件批处理。
- **actions**: 自定义应用动作支持灵活的上下文过滤规则，可按「全部文件与文件夹」、「仅文件夹」、「仅文件」或「指定文件扩展名（如 zip, 7z, rar）」精确控制右键激活时机。
- **ui**: 新增「自定义应用」偏好管理面板与配置编辑弹窗，支持读取本地应用的真实高清系统图标与 Bundle 标识符，支持快速启停与删除。
- **canvas**: 自定义应用动作无缝并入动作资源库与菜单编排画布，支持拖拽排序、自由加入一级或二级菜单。
- **ipc**: 自定义应用配置持久化至 App Group / 扩展共享容器 `custom_app_actions.json`，并由分布式通知机制实时驱动访达扩展热重载。
...
- **finder**: 为 iCloud Drive、文稿和其他 File Provider 保护目录补充系统服务直达动作；收藏优先、常用动作补齐，最多展示 8 项，并保留“EasyRight…”完整分类面板。

### Packaging

- **homebrew**: Cask 更新到 v1.2.1 不可变 DMG 及远程制品的真实 SHA-256。
- **homebrew**: 移除会使升级中断的 FinderSync 注销脚本；为已安装 v1.2.0 的用户补充一次性 Cask 收据迁移指引，并在 Cask 输出中直接显示可执行的恢复命令。
...
- **ext**: 「一键注册扩展」现在自动执行 `pluginkit -a` 注册 + `pluginkit -e use -i com.easyright.app.extension` 启用 + `killall Finder` 重启访达三步，不再仅注册不生效（这是过去用户反复踩到的「等了一会没生效」的根因）。
...
- **logging**: 全局日志切 OSLog（`subsystem == "com.easyright.app"`），按 category 区分 `host` / `ext` / `storage` / `action` / `ui`；废弃 `extension.log` 文件追加路径。
...
完整提交记录见 [GitHub Releases v1.0.1](https://github.com/easyright/EasyRight/releases/tag/v1.0.1)。
...
[v1.0.1]: https://github.com/easyright/EasyRight/releases/tag/v1.0.1
[v0.0.2]: https://github.com/easyright/EasyRight/releases/tag/v0.0.2
[v0.0.1]: https://github.com/easyright/EasyRight/releases/tag/v0.0.1
