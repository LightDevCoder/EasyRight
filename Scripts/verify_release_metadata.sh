...
if [[ -z "${VERSION_OVERRIDE:-}" || "${VERIFY_BUILD_ARTIFACTS:-0}" == "1" ]]; then
  check_plist_version "$ROOT_DIR/build/EasyRight.app/Contents/Info.plist" "主程序"
  check_plist_version "$ROOT_DIR/build/EasyRight.app/Contents/PlugIns/EasyRightExtension.appex/Contents/Info.plist" "Finder 扩展"
fi
