...
CASK_FILE="${1:-$ROOT_DIR/Casks/easyright.rb}"
...
if [[ "${CASK_VERIFY_DOWNLOAD:-1}" == "1" ]]; then
    ...
    DOWNLOAD_URL="https://github.com/easyright/EasyRight/releases/download/v${VERSION_VALUE}/EasyRight-v${VERSION_VALUE}-macOS-Universal.dmg"
    TEMP_DMG="$(mktemp -t easyright-cask).dmg"
...
if command -v brew >/dev/null 2>&1; then
    TAP_NAME="${HOMEBREW_TAP_NAME:-easyright/easyright}"
    ...
    if [[ -n "$TAP_REPO" && "$CASK_FILE" == "$TAP_REPO/"* ]]; then
        ...
        brew audit --cask --strict --online "$TAP_NAME/easyright"
    else
        echo "⚠️ [Cask] Homebrew 要求 style/audit 的 Cask 位于 tap 中。"
        echo "⚠️ [Cask] 当前路径不是 $TAP_NAME tap，已跳过 brew style/audit。"
        echo "⚠️ [Cask] 可在发布后运行: brew tap $TAP_NAME https://github.com/easyright/EasyRight.git"
    fi
else
